"""
Streamlit dashboard for poultry production monitoring and anomaly analysis.

The application visualizes the datasets generated by the local Lakehouse
reconstruction pipeline and provides two complementary monitoring views:

- Production: daily production KPIs, battery-level comparisons, temporal
  production trends and spatial production heatmaps.
- Anomalies: inspection of temporal and persistent spatial anomalies,
  including affected cages and historical evolution.

The dashboard is designed as the visualization layer of the Poultry
Edge-to-Cloud prototype and consumes preprocessed production and anomaly
datasets stored under ``data/dashboard``.
"""
...
# Resolve data paths relative to the repository root so the application
# can be launched independently of the current working directory.
...
# Apply lightweight custom styling to metrics and status messages while
# keeping the standard Streamlit layout and components.
st.markdown(
    """
    <style>
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Load and prepare the datasets required by the dashboard.

    The production and anomaly CSV files are loaded from the local
    dashboard data directory. Date fields are converted to pandas
    timestamps and physical location identifiers are normalized to
    nullable integer columns.

    Streamlit caches the resulting DataFrames to avoid reloading and
    preprocessing the files after every interface interaction.

    Returns
    -------
    tuple[pd.DataFrame, pd.DataFrame]
        Production monitoring data and detected anomaly data.

    Raises
    ------
    FileNotFoundError
        If either of the required dashboard datasets is missing.
    """
    ...
    # Parse dates once during data loading so all subsequent filters
    # and visualizations operate on consistent timestamp values.
    ...
    # Normalize physical hierarchy identifiers while preserving missing
    # values through pandas nullable integer types.
...
# Stop the application early if the dashboard datasets cannot be loaded.
try:
...
def battery_daily_table(df: pd.DataFrame) -> pd.DataFrame:
    """
    Extract one aggregated production record per battery and day.

    The production dashboard contains spatially resolved rows, meaning
    battery-level metrics are repeated across several spatial groups.
    This helper removes those repetitions before battery-level KPIs and
    temporal trends are calculated.

    Parameters
    ----------
    df:
        Production DataFrame containing battery and spatial metrics.

    Returns
    -------
    pd.DataFrame
        One record per farm, house, battery and capture date.
    """
    cols = [
...
def format_int(value) -> str:
    """Format a numeric value as an integer for dashboard display."""
    return "—" if pd.isna(value) else f"{int(round(value)):,}"


def format_pct(value, decimals=1) -> str:
    """Format a numeric value as a percentage for dashboard display."""
    return "—" if pd.isna(value) else f"{value:.{decimals}f}%"


def format_float(value, decimals=3) -> str:
    """Format a numeric value with a configurable number of decimals."""
    return "—" if pd.isna(value) else f"{value:.{decimals}f}"
...
def temporal_anomalies_for_selection(farm_id, selected_date, house_number=None):
    """
    Return temporal anomalies matching the current production selection.

    Results are filtered by farm and date and can optionally be restricted
    to a specific house.
    """
    df = anomalies[
...
def spatial_anomalies_for_selection(farm_id, house_number=None):
    """
    Return persistent spatial anomalies matching the current selection.

    Results are filtered by farm and can optionally be restricted to a
    specific house.
    """
    df = anomalies[
...
# Safely manage navigation between the Production and Anomaly views.
# Navigation requests are applied before the sidebar radio widget is
# instantiated to avoid modifying an already-created widget state.
if "page_selector" not in st.session_state:
    st.session_state["page_selector"] = "Producción"

# Apply a navigation request from the previous rerun before creating
# the sidebar radio widget.
...
if page == "Producción":
    ...
    # Check whether the current production selection contains temporal
    # or persistent spatial anomalies and expose direct navigation to
    # their detailed inspection.
    ...
    # Battery metrics are weighted by the number of records so the overall
    # mean represents the complete selected population.
    ...
    # ------------------------------------------------------------
    # Battery-level production comparison
    # ------------------------------------------------------------
    ...
    # Build a readable physical identifier combining house and battery.
    ...
    # ------------------------------------------------------------
    # Temporal production monitoring
    # ------------------------------------------------------------

    # Compare the observed daily production of the selected battery with
    # its historical baseline and highlight previously detected anomalies.
    ...
    # ------------------------------------------------------------
    # Spatial production monitoring
    # ------------------------------------------------------------

    # Aggregate production by physical level and groups of cage positions
    # to reproduce the spatial organization of the selected battery.
    ...
    if heat_df.empty:
        ...
        # Build a level × position-group matrix used as the heatmap input.
        ...
        # Map each heatmap cell to the real cage identifiers represented
        # by that spatial group.
        hover_matrix = []
        ...
        # Physical battery orientation: level 1 at the bottom and higher
        # levels progressively above it.
        fig.update_yaxes(
            title="Nivel",
            tickmode="array",
            tickvals=levels,
...
# ============================================================
    ...
    # Provide detailed inspection of the temporal and spatial anomalies
    # previously detected by the analytical pipeline.
    ...
    # Restore the anomaly selected from the Production view so the
    # corresponding filters can be initialized automatically.
    ...
    # Temporal anomalies represent battery-level production decreases
    ...
    # Spatial anomalies identify persistent low-production cage groups
    # relative to the remaining spatial groups within the same battery.
    ...
    if spatial.empty:
        ...
        # Allow detailed inspection of the physical battery containing the
        # selected spatial anomaly.
        ...
        if not spatial_candidates.empty:
            ...
            if not history.empty:
                # Apply the same spatial anomaly criteria used during the
                # analytical detection stage to identify anomalous days in
        ...
        # ------------------------------------------------------------
        # Detailed battery heatmap
        # ------------------------------------------------------------

        # Reconstruct the selected battery as a level × position-group heatmap
        # and highlight the spatial regions associated with detected anomalies.
        ...
        # Overlay rectangles on the heatmap to indicate the physical
        # location of the detected anomalous cage groups.
